...
install_zabbix_agent() {
    ...
    if command -v yum >/dev/null 2>&1; then
        rpm -Uvh https://repo.zabbix.com/zabbix/6.4/rhel/7/x86_64/zabbix-release-latest-6.4.el7.noarch.rpm
        #yum clean all
...
update_zabbix_config() {
    ...
    #tar xf /etc/zabbix.tar.gz -C /etc/
    config_file="/etc/zabbix/zabbix_agent2.conf"
    # if [[ ! -f "$config_file" ]]; then
    #     echo "配置文件 $config_file 未找到！请检查 Zabbix-Agent 是否安装正确。"
    #     exit 1
    # fi 
